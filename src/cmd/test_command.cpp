...
nlohmann::json to_json(const JudgeResult &r) {
  return {
      {"verdict", r.verdict_str()},         {"time_ms", r.time_ms},
      {"memory_kb", r.memory_kb},           {"message", ensure_utf8(r.message)},
      {"input", ensure_utf8(r.input)},      {"output", ensure_utf8(r.output)},
...
// (Copied from main.cpp, we might want to move these to a shared util if
// reused)
...
// ... (We need the implementation of infer_spec_from_db_cases and others from
// main.cpp if we fully implement dynamic testing) For brevity and to ensure
// compilation, I will implement essential parts and reuse logic.
...
void cmd_test(CommandContext &ctx) {
  try {
    ...
    if (!fs::exists(src_file)) {
      std::cerr << "[!] 找不到代码文件: " << src_file << std::endl;
      return;
    }
    ...
    // C. Dynamic Generation (Only if requested or essentially needed for
    // checking?) The prompt asks for "Enhanced list function" and "view
    // details". It says "Detailed progress info: Passed X/Y test points". We
    // should run the collected cases. Note: The original cmd_test had "Stress
    // Mode" (infinite loop) vs "Static Mode". The requirement implies a finite
    // set of tests mainly. If user wants stress test, maybe `test --stress`?
    ...
    if (cases.empty()) {
      ...
      if (!has_scripts) {
        if (svc.ai->enabled()) {
          std::cout << "[*] 正在调用 AI 生成对拍脚本 (gen.py / sol.py)..."
                    << std::endl;
          try {
            ...
            if (scripts.first.empty() || scripts.second.empty()) {
              ...
              std::ofstream fg(gen_py);
              fg << scripts.first;
              std::ofstream fs(sol_py);
              fs << scripts.second;
              std::cout << "[+] 脚本已保存至 " << validator_dir.string()
                        << std::endl;
              has_scripts = true;
            }
          } catch (const std::exception &e) {
          ...
          std::cout << "[!] AI 未启用 (请配置 api_key)，跳过脚本生成。"
                    << std::endl;
        }
      }
      ...
      if (has_scripts) {
        ...
        for (int i = 1; i <= gen_count; ++i) {
          ...
          fs::path in_path =
              prob_dir / "temp" / ("gen_" + std::to_string(i) + ".in");
          fs::path ans_path =
              prob_dir / "temp" / ("gen_" + std::to_string(i) + ".ans");

          // Run Generator
          // python "gen_py" > in_path
          // We use run_process_redirect which handles redirection internally if
          // supported, or we pass the command. Judge::run_process_redirect(cmd,
          // input_file, output_file, ...)
          std::string cmd_gen = "python \"" + gen_py.string() + "\"";
          auto res_gen = svc.judge->run_process_redirect(
              cmd_gen, "", in_path.string(), 5000, 256 * 1024);

          if (res_gen.verdict != Verdict::AC) {
            std::string msg = res_gen.message.empty() ? res_gen.verdict_str()
                                                      : res_gen.message;
            std::cerr << "[!] 生成器运行失败 (Case " << i << "): " << msg
                      << std::endl;
            continue;
          }
          ...
          auto res_sol = svc.judge->run_process_redirect(
              cmd_sol, in_path.string(), ans_path.string(), 5000, 256 * 1024);

          if (res_sol.verdict != Verdict::AC) {
            std::string msg = res_sol.message.empty() ? res_sol.verdict_str()
                                                      : res_sol.message;
            std::cerr << "[!] 标程运行失败 (Case " << i << "): " << msg
                      << std::endl;
            continue;
          }
    ...
    if (cases.empty()) {
      std::cout << "[!] 无法获取任何测试用例 (静态/数据库/AI生成)。"
                << std::endl;
    }

    report.total_count = cases.size();
    std::cout << "=== 开始测试 (共 " << cases.size()
              << " 个测试点) ===" << std::endl;
    ...
    for (size_t i = 0; i < cases.size(); i++) {
      ...
      std::cout << " (" << res.time_ms << "ms, " << res.memory_kb << "KB)   "
                << std::endl; // Extra spaces to clear "Running..."
    }

    report.pass_count = passed;
    report.verdict =
        all_ac ? "AC" : (passed > 0 ? "WA" : "WA"); // WA if any failed
    // Refine verdict if TLE/RE?
    // Just use "WA" or specific if all failed same way?
    // Let's iterate report.cases to find first non-AC for the summary verdict
    // if not AC
    if (!all_ac) {
      for (const auto &c : report.cases) {
    ...
    // Update DB
    svc.db->update_problem_status(prob.id, report.verdict, report.pass_count,
                                  report.total_count);

    svc.judge->cleanup_prepared(user_exe, svc.cfg.language);

    // AI Diagnosis
    // std::cout << "[DEBUG] AI Enabled: " << svc.ai->enabled() << ", All AC: "
    // << all_ac << std::endl;
    if (!all_ac && svc.ai->enabled()) {
      std::cout << std::endl
                << "[*] 测试未通过，正在进行AI分析..." << std::endl;
      ...
      for (const auto &c : report.cases) {
        if (c.verdict != Verdict::AC) {
          failure_info =
              fmt::format("Verdict: {}\nInput:\n{}\nOutput:\n{}\nExpected:\n{}",
                          c.verdict_str(), c.input.substr(0, 200),
                          c.output.substr(0, 200), c.expected.substr(0, 200));
          break;
        }
      }
      ...
      // Streaming output callback with filtering (Reuse logic? Copy for now to
      // be safe)
      std::string buffer;
      bool suppress = false;
