...
> 这是一次包含核心机制重构和重大漏洞修复的更新版本。我们深度优化了项目的 TUI 视觉版式与导航体验，重构了底层目录依赖逻辑，并重新设计了更健壮的刷题闭环。

### 新增功能 (New Features)
- **TUI 视觉效果升级**：优化了 TUI 欢迎页的视觉布局，引入由 ASCII 手绘的简约图案模块，并排布为更加清爽的双列（系统信息与命令快查表）宽屏结构。
- **列表组合检索功能**：重构并新增了支持多条件（如按系统难度、爬虫来源、过滤状态集等多维度）并行叠加的组合题项搜索引擎，摆脱了以前繁复单一的视图轮询模式。
- **全平台难度字符串规范化**：将跨越洛谷、LeetCode、蓝桥、Codeforces 平台的多种原始难度文本在底层统一映射标准化至 `easy`、`medium`、`hard` 体系，确保持续的难度分类检索与色彩视觉一致性。

### 修复问题 (Bug Fixes)
- **解决核心流程并发死锁阻断**：移除了 TUI 互动流上下游中会造成控制面板锁定（以及等待获取终端权限）旧版阻塞函数与 `system()` 判定。全面采用增强型 `CallbackStreamBuf` 拦截处理手段。
- **防止 TUI 交互式输入干扰污染**：对 `/login` 等在内的涉及敏感复制黏贴验证机制命令施加约束，引导用户至相对安全的纯 CLI 层独立校验以根除了图形界面随机挂起或崩溃的顽疾；并对破坏性命令 `/delete` 等要求强确认机制。
- **子菜单导航堆栈状态记忆**：重构修复了自 `HintView` 等多层展示页通过 `Esc` 返回操作导致 TUI 一并退回最初页面，丢失检索筛选与光标锚点进度的使用体验问题。
- **补齐本地题意指针偏离**：解决了通过 `cmd_new` 手动创建题目所产生的非规范 `.md` 文档与 DB 默认的 `content_path` 指针定位不匹配，进而让后来 `solve/hint` 内文不显示的 Bug。

### 破坏性变更与架构重构 (Breaking Changes & Architecture Refactor)
- **打通最新刷题机制入口**：全线淘汰掉存在交互与判断歧义机制隐患的旧版指令 `submit`。所有涉及复习回顾与打分错推逻辑正式转由新加入并适配防阻塞引擎的 `/record <id>` 单一接管替换。
- **底层文件锚定迁移 (UUID)**：将曾经由于强关联隐式相对路径和 SQLite 的漂移主键 `rowid/display_id` 管理架构全面解耦废弃。解题源文件（`solution`）与测评报告等周边存储体系彻底切为由 `uuid` + `Project Root` 绝对地址直接操控，彻底避免任意切区层报错缺失或外键插入等灾难风险。
- **应用基座层 (Project Utilities) 重塑隔离**：重构并移除散落模块代码各处中的探针级根目录寻址控制实现。归一抽象成更明晰的 `shuati::utils::project_utils` 等工具接口统一驱动。
...
### 变更概览
- **TUI 稳定性修复**：为命令执行引入串行队列与生命周期保护，避免快速退出/切换视图后出现崩溃；完善输出滚动与流式输出的刷新行为（包括 `\r`）。
- **TUI 使用体验增强**：主输出面板可视窗口从 10 行提升到 20 行，并同步 PageUp/PageDown 滚动上限。
- **`python` 判题功能恢复**：统一通过标记驱动在 sandbox 中解析并执行 `python`（基于 PATH 的可执行文件解析），确保 TUI/`/test` 等流程可正常工作。
- **代码健壮性增强**：对 `std::cout/std::cerr` 的全局重定向加锁，减少并发时序问题；Linux sandbox 在 PATH 场景下使用更合适的 `execvp` 行为。
- **测试与安全性**：扩展 `test_judge_security`，补齐临时可执行文件清理的覆盖（含 unicode 路径）。

> 特别说明： VS Code 用户推荐使用 v0.0.7，该版本针对 VS Code 集成终端的兼容性优化最佳。后续版本正在持续改进中。
---
